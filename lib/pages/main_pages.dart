...
class _MainPagesState extends State<MainPages> with WidgetsBindingObserver {
		...
		G.rt.showChatPage = (MsgBean msg) {
			...
			if (G.rt.enableNotification) {
				if (UserAccount.notificationIdMap.containsKey(msg.keyId())) {
					_cancelNotification(
						UserAccount.notificationIdMap[msg.keyId()]);
				}
			}
				...
				children: [
					Container(
						constraints: BoxConstraints(
							maxWidth: G.rt.chatListFixedWidth),
	...
	Widget build(BuildContext context) {
		...
		bool hori = MediaQuery
			...
			MediaQuery
				.of(context)
				.size
				.width > G.rt.chatListFixedWidth * 1.5;
	...
	void _initNotifications() {
		...
		AwesomeNotifications().actionStream.listen((receivedNotification) {
			...
			print(
				'notification chatId: $keyId, keyButton: ${receivedNotification
					.buttonKeyPressed}, keyInput:${receivedNotification
					.buttonKeyInput}');
	...
	Widget createScaffoldPage(BuildContext context, Widget widget,
	                          String title) {
		return Scaffold(
			appBar: AppBar(
